...
const getInitialState = (): HomeScreenState => {
  ...
  const page1: Page = {
    id: 'page-1',
    items: [
      { id: 'widget-profile-initial', type: 'widget', widgetType: 'profile', size: '2x2' },
      { id: 'widget-calendar-initial', type: 'widget', widgetType: 'calendar', size: '2x2' },
      ...firstPageApps.filter(app => !['phone', 'safari', 'messages', 'spotify'].includes(app.id))
                      .slice(0, 14)
                      .map(app => ({ id: `app-${app.id}-${Math.random()}`, type: 'app' as const, appId: app.id }))
    ]
  };

  const initialDockItems = DOCK_APPS.map(app => ({ id: `app-${app.id}-dock-${Math.random()}`, type: 'app' as const, appId: app.id }));

  return { pages: [page1], dockItems: initialDockItems };
};
...
interface HomeScreenContextType {
    ...
    updateWidget: (itemId: string, updates: { type?: string; size?: WidgetSize }) => boolean;
...
export const HomeScreenProvider = ({ children }: { children: ReactNode }) => {
    const [homeScreenState, setHomeScreenState] = useState<HomeScreenState>({ pages: [], dockItems: [] });
    const [visiblePages, setVisiblePages] = useState<boolean[]>([]);
    const [lastAddedPageIndex, setLastAddedPageIndex] = useState<number | null>(null);
    ...
    useEffect(() => {
        if (homeScreenState.pages.length > 0 || homeScreenState.dockItems.length > 0) {
            localStorage.setItem('homeScreenState', JSON.stringify(homeScreenState));
        }
    }, [homeScreenState]);

    const acknowledgeNavigation = useCallback(() => setLastAddedPageIndex(null), []);

    const findItem = useCallback((itemId: string): FindResult | null => {
        for (let i = 0; i < homeScreenState.pages.length; i++) {
            ...
            if (itemIndex !== -1) {
                return { item: page.items[itemIndex], containerId: page.id, pageIndex: i, itemIndex };
            }
        }
        const dockItemIndex = homeScreenState.dockItems.findIndex(item => item.id === itemId);
        if (dockItemIndex !== -1) {
            return { item: homeScreenState.dockItems[dockItemIndex], containerId: 'dock', itemIndex: dockItemIndex };
        }
        return null;
    }, [homeScreenState]);
    
    const moveItem = useCallback((activeId: string, overId: string, overData: any): boolean => {
        let success = false;
        setHomeScreenState(produce(draft => {
            const findItemLocation = (d: typeof draft, id: string) => {
                const dockIndex = d.dockItems.findIndex(item => item.id === id);
                if (dockIndex !== -1) {
                    return { container: d.dockItems, containerId: 'dock', index: dockIndex };
                }
                for (const page of d.pages) {
                    const itemIndex = page.items.findIndex(item => item.id === id);
                    if (itemIndex !== -1) {
                        return { container: page.items, containerId: page.id, index: itemIndex };
                    }
                }
                return null;
            };
            ...
            const { container: sourceContainer, containerId: sourceContainerId, index: sourceIndex } = activeLocation;

            const overIsContainer = overData?.type === 'container';
            const overLocation = overIsContainer ? null : findItemLocation(draft, overId);
            const destContainerId = overIsContainer ? overId : overLocation?.containerId;
            if (!destContainerId) return;

            if (sourceContainerId === destContainerId) {
                if (!overLocation) return;
                const newItems = arrayMove(sourceContainer, sourceIndex, overLocation.index);
                ...
                const destContainer = (destContainerId === 'dock') ? draft.dockItems : draft.pages.find(p => p.id === destContainerId)?.items;
                if (!destContainer) {
                ...
                if (destContainerId !== 'dock' && getPageCapacity(destContainer) + getItemSize(movedItem) > MAX_PAGE_CAPACITY) {
                    sourceContainer.splice(sourceIndex, 0, movedItem);
                    return;
                }
                
                if (overLocation) { // Dropped on an item
                    if (destContainerId === 'dock' && destContainer.length >= MAX_DOCK_ITEMS) {
                        const [swappedItem] = destContainer.splice(overLocation.index, 1, movedItem);
                        sourceContainer.push(swappedItem);
                    } else {
                        destContainer.splice(overLocation.index, 0, movedItem);
                    }
                } else { // Dropped on a container's background
                    if (destContainerId === 'dock' && destContainer.length >= MAX_DOCK_ITEMS) {
                        sourceContainer.splice(sourceIndex, 0, movedItem);
                        return;
                    }
    ...
    const addApp = useCallback((appId: string) => {
      ...
      const newHomeScreenState = produce(homeScreenState, draft => {
        const newApp: GridItem = { id: `app-${appId}-${Date.now()}`, type: 'app', appId };
        
        let pageAdded = false;
        const visiblePageIndices = draft.pages.map((p, i) => visiblePages[i] ? i : -1).filter(i => i !== -1);
      ...
      if (pageIndex !== -1) {
        ...
        const addWidget = useCallback((widgetType: string, requestedPageIndex: number): void => {
        ...
        const visiblePageIndices = homeScreenState.pages.map((_, i) => i).filter(i => visiblePages[i]);
        ...
        for (const idx of indicesToCheck) {
            if (idx >= 0 && homeScreenState.pages[idx]) {
                 if (getPageCapacity(homeScreenState.pages[idx].items) + getItemSize(newWidget) <= MAX_PAGE_CAPACITY) {
                    setHomeScreenState(produce(draft => {
        ...
        setHomeScreenState(current => ({ ...current, pages: [...current.pages, newPage] }));
        setVisiblePages(currentVisibility => [...currentVisibility, true]);
        setLastAddedPageIndex(newPageIndex);
    }, [homeScreenState, visiblePages]);

    const removeItem = useCallback((itemId: string) => {
        const findItemLocation = (draft: HomeScreenState, itemId: string): { container: GridItem[]; index: number } | null => {
            const dockIndex = draft.dockItems.findIndex(item => item.id === itemId);
            if (dockIndex !== -1) return { container: draft.dockItems, index: dockIndex };
            for (const page of draft.pages) {
                const itemIndex = page.items.findIndex(item => item.id === itemId);
                if (itemIndex !== -1) return { container: page.items, index: itemIndex };
            }
            return null;
        }
    ...
    const updateWidget = useCallback((itemId: string, updates: { type?: string; size?: WidgetSize }): boolean => {
        let success = false;
                ...
                if (item.type === 'widget') {
                    ...
                    if (getPageCapacity(tempItems) <= MAX_PAGE_CAPACITY) {
                         if (updates.type) {
                            ...
                            const widgetConfig = WIDGETS[updates.type as keyof typeof WIDGETS];
                            if (widgetConfig && item.size && !widgetConfig.supportedSizes.includes(item.size)) {
                        ...
                        if (updates.size) {
                            ...
                            const widgetConfig = WIDGETS[widgetType as keyof typeof WIDGETS];
                            if (widgetConfig && widgetConfig.supportedSizes.includes(updates.size)) {
    ...
    return React.createElement(HomeScreenContext.Provider, { value: value, children: children });
};
